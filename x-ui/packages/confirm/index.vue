      ...
      <div class="mzl-wrapper">
        ...
        <div class="mzl-body">
          <!-- <i :class="['icon-warning',icon]"></i> -->
           <x-icon :code="icon" size="1"></x-icon>
          <span>{{text}}</span>
        </div>
        <div class="mzl-footer">
          <x-button @click="cancelCallback" size="small" style="margin-right:10px" v-if="cancelShow">{{cancelText}}</x-button>
          <x-button @click="confirmCallback" size="small" type="primary" v-if="confirmShow">{{confirmText}}</x-button>
        </div>
      </div>
